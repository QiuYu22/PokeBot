...
namespace SysBot.Pokemon.WinForms
{
    public partial class BotController : UserControl
    {
        ...
        public void SendCommand(BotControlCommand cmd, bool echo = true)
        {
            ...
            {
                LogUtil.LogError( "数据中枢","由于跳过控制台机器人创建已开启，未创建任何机器人！");
                return;
            }
            var bot = GetBot();
            if (bot is null)
        ...
        private void ExecuteScreenCommand(bool screenOn)
        {
            if (Runner is null)
            {
                LogUtil.LogError("运行器为空 - 无法执行屏幕命令", "机器人控制器");
                return;
            }
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        LogUtil.LogError("没有可用的机器人执行屏幕命令", "机器人控制器");
                        return;
                    }
                    ...
                    foreach (var botSource in bots)
                    {
                        try
                        {
                            ...
                            if (bot?.Connection != null && bot.Connection.Connected)
                            {
                                ...
                                successCount++;
                                LogUtil.LogInfo($"已为 {bot.Connection.Name} {(screenOn ? "开启" : "关闭")}屏幕", "机器人控制器");
                            }
                            else
                            {
                    ...
                    LogUtil.LogInfo($"已向 {successCount}/{totalCount} 个机器人发送屏幕命令", "机器人控制器");
                }
                catch (Exception ex)
                {
        ...
        public string ReadBotState()
        {
            ...
            {
                LogUtil.LogError($"读取机器人状态出错：{ex.Message}", "机器人控制器");
                return "ERROR";
            }
        }
        ...
        public BotSource<PokeBotState>? GetBot()
        {
            ...
            {
                LogUtil.LogError($"获取机器人出错：{ex.Message}", "机器人控制器");
                return null;
            }
        }
                ...
                if (!_hasPendingStateUpdate)
                {
                    ...
                    BeginInvoke((System.Windows.Forms.MethodInvoker)(() => 
                    {
                        ...
                        if (!_suspendPainting && !IsDisposed)
                        {
                            ...
                            {
                                LogUtil.LogError($"更新机器人状态出错：{ex.Message}", "机器人控制器");
                            }
                        }
                    }));
                }
