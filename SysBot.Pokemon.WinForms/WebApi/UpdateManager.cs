...
public static class UpdateManager
{
    ...
    public class InstanceIdleStatus
    {
        ...
        public string Name => IsMaster ? "主控" : $"实例 {TcpPort}";
    ...
    public static void SetConfiguredWebPort(int port)
    {
        ...
        {
            LogUtil.LogError("更新管理器", $"Web 端口 {port} 无效，已回退到 8080");
            port = 8080;
        }
    ...
    public static UpdateState? GetCurrentState()
    {
        lock (_lock)
        {
            ...
            try
            {
                ...
                {
                    LogUtil.LogError("更新管理器", "状态文件过大，疑似遭受 DoS 攻击");
                ...
                {
                    LogUtil.LogInfo("更新管理器", $"检测到 {_state.StartTime:u} 的过期更新状态，将其标记为失败");
            ...
            {
                LogUtil.LogError("更新管理器", $"读取更新状态失败：{ex.Message}");
                try { File.Delete(safePath); } catch { }
                return null;
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(currentTcpPort), "端口必须位于 1-65535 之间");
        }

        LogUtil.LogInfo("更新管理器", "正在启动更新流程");
        ...
        lock (_lock)
        {
            ...
            {
                LogUtil.LogInfo("更新管理器", $"恢复未完成的更新会话 {existingState.SessionId}");
                ...
                _ = Task.Run(async () => 
                {
                    ...
                    {
                        LogUtil.LogError("更新管理器", $"恢复更新失败：{ex.Message}");
        ...
        _ = Task.Run(async () => 
        {
            ...
            {
                LogUtil.LogError("更新管理器", $"执行更新失败：{ex.Message}");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            LogUtil.LogError("更新管理器", $"端口号 {instancePort} 无效");
            return false;
        }

        LogUtil.LogInfo("更新管理器", $"开始更新端口 {instancePort} 的单实例");
        ...
        {
            LogUtil.LogError("更新管理器", $"端口 {instancePort} 的单实例更新超时");
            return false;
        }
        catch (Exception ex)
        {
            LogUtil.LogError("更新管理器", $"单实例更新失败：{ex.Message}");
            return false;
        }
        finally
        {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                CompleteUpdate(state, true, "没有可用的更新");
                return;
            }
            ...
            LogUtil.LogInfo("更新管理器", $"更新顺序：从属端口 {string.Join(", ", slaves.Select(s => s.TcpPort))}，主控端口 {master?.TcpPort ?? 0}");
            ...
            foreach (var slave in slaves)
            {
                ...
                LogUtil.LogInfo("更新管理器", $"正在更新端口 {slave.TcpPort} 的从属实例");
                SaveState();
            ...
            if (master != null)
            {
                state.CurrentUpdatingInstance = "主控";
                state.Message = "正在更新主控实例（将触发重启）…";
                LogUtil.LogInfo("更新管理器", $"正在更新主控端口 {master.TcpPort}，该操作将重启程序");
                SaveState();
            ...
            CompleteUpdate(state, allSuccess, allSuccess ? "更新已全部成功完成" : "更新已完成但存在错误");
        }
        catch (OperationCanceledException)
        {
            CompleteUpdate(state, false, "更新已被取消");
        }
        catch (Exception ex)
        {
            LogUtil.LogError("更新管理器", $"更新失败：{ex}");
            CompleteUpdate(state, false, $"更新失败：{ex.Message}");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            LogUtil.LogInfo("更新管理器", $"正在从阶段 {state.Phase} 恢复更新");
            ...
            if (state.Phase == UpdatePhase.Updating)
            {
                ...
                if (master != null)
                {
                    ...
                    LogUtil.LogInfo("更新管理器", $"重启后检测主控：当前版本={currentVersion}，目标版本={state.TargetVersion}，主控状态={master.Status}");
                    ...
                    if (currentVersion == state.TargetVersion)
                    {
                        if (master.Status != InstanceStatus.Completed)
                        {
                            ...
                            LogUtil.LogInfo("更新管理器", $"主控更新验证成功，当前运行版本为 {currentVersion}");
                            SaveState();
                        }
                    }
                    else if (master.Status == InstanceStatus.Pending)
                    {
                        ...
                        LogUtil.LogError("更新管理器", $"主控更新失败：期望 {state.TargetVersion}，实际 {currentVersion}");
                        SaveState();
                    }
                }
            }
            ...
            var allSuccess = state.Instances.All(i => i.Status == InstanceStatus.Completed);
            CompleteUpdate(state, allSuccess, allSuccess ? "更新恢复后已成功完成" : "更新恢复后仍有错误");
        }
        catch (Exception ex)
        {
            LogUtil.LogError("更新管理器", $"恢复失败：{ex}");
    ...
    private static async Task<List<Instance>> DiscoverInstancesSimpleAsync(int currentTcpPort)
    {
                        ...
                        if (await Task.WhenAny(writeTask, Task.Delay(500)) == writeTask)
                        {
                            ...
                            if (await Task.WhenAny(readTask, Task.Delay(500)) == readTask)
                            {
                                ...
                                if (!string.IsNullOrEmpty(response) && response.StartsWith('{'))
                                {
                                    ...
                                    {
                                        LogUtil.LogError("更新管理器", $"解析端口 {portToScan} 的信息响应失败：{ex.Message}");
                                    }
                                }
                            }
                        }
        ...
        {
            LogUtil.LogError("更新管理器", $"发现本地实例时出错：{ex.Message}");
        }
        ...
        if (!currentIsMaster && instances.Count > 1)
        {
            ...
            if (masterPort > 0)
            {
                ...
                if (masterInstance != null)
                {
                    ...
                    LogUtil.LogInfo("更新管理器", $"已识别到托管 Web 服务的主控实例，TCP 端口 {masterPort}");
                }
            }
        }
        ...
        if (!instances.Any(i => i.IsMaster))
        {
            ...
            if (defaultMaster != null)
            {
                ...
                LogUtil.LogInfo("更新管理器", $"未找到主控，使用默认 TCP 端口 {Config.MasterTcpPort}");
            }
        }
        ...
        {
            LogUtil.LogInfo("更新管理器", $"已发现 {instances.Count} 个实例：");
            foreach (var inst in instances)
            {
                LogUtil.LogInfo("更新管理器", $"  - 端口 {inst.TcpPort}: 主控={inst.IsMaster}, 版本={inst.Version}");
            }
        }
        return instances;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        LogUtil.LogInfo("更新管理器", $"正在停止所有机器人（超时 {timeout.TotalMinutes} 分钟）");
        ...
        foreach (var instance in instances)
        {
            ...
            {
                LogUtil.LogError("更新管理器", $"端口 {instance.TcpPort} 停止机器人失败：{ex.Message}");
            }
        }
        ...
        while (DateTime.UtcNow < endTime && !cancellationToken.IsCancellationRequested)
        {
            ...
            if (_state?.IdleProgress != null)
            {
                ...
                _state.Message = $"正在等待机器人待机：{_state.IdleProgress.IdleBots}/{_state.IdleProgress.TotalBots}";
                SaveState();
            }
            else
            {
            ...
            if (allStopped)
            {
                LogUtil.LogInfo("更新管理器", "全部机器人已成功待机");
                if (_state != null)
                {
                    _state.Message = "所有机器人均已成功待机";
                    SaveState();
                }
                return;
            }
        ...
        LogUtil.LogError("更新管理器", "停止机器人超时，继续执行后续流程");
        if (_state != null)
        {
            _state.Message = "等待机器人待机超时，强制继续更新";
            SaveState();
        }
    }
    ...
    private static async Task UpdateInstanceIdleStatus(Main? mainForm, Instance instance, InstanceIdleStatus idleStatus, CancellationToken cancellationToken)
    {
        ...
        {
            LogUtil.LogError("更新管理器", $"更新端口 {instance.TcpPort} 的待机状态失败：{ex.Message}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int retry = 0; retry < Config.MaxRetryCount; retry++)
        {
            try
            {
                ...
                LogUtil.LogInfo("更新管理器", $"正在更新端口 {instance.TcpPort}（第 {retry + 1}/{Config.MaxRetryCount} 次尝试）");
                ...
                LogUtil.LogInfo("更新管理器", $"端口 {instance.TcpPort} 的实例更新成功");
                return;
            }
            catch (Exception ex)
            {
                LogUtil.LogError("更新管理器", $"第 {retry + 1} 次更新尝试失败：{ex.Message}");
    ...
    private static async Task WaitForProcessRestartAsync(Instance instance, CancellationToken cancellationToken)
    {
        ...
        while (DateTime.UtcNow < startEndTime)
        {
            ...
            {
                LogUtil.LogInfo("更新管理器", $"端口 {instance.TcpPort} 的新进程已启动");
                return;
            }
        ...
        throw new TimeoutException("新进程未在规定时间内启动");
    }
    ...
    {
        LogUtil.LogInfo("更新管理器", $"更新流程结束：{message}");
    ...
    private static void SaveState()
    {
        ...
        {
            LogUtil.LogError("更新管理器", $"保存更新状态失败：{ex.Message}");
        }
    }
    ...
    public static void ClearSession()
    {
        lock (_lock)
        {
            ...
            if (File.Exists(StateFilePath))
            {
                ...
                {
                    File.Delete(StateFilePath);
                    LogUtil.LogInfo("更新管理器", "更新会话已清除");
                }
                catch (Exception ex)
                {
                    LogUtil.LogError("更新管理器", $"删除状态文件失败：{ex.Message}");
                }
            }
        }
    }
    ...
    public static void ForceCompleteSession()
    {
        lock (_lock)
        {
            ...
            if (state != null && !state.IsComplete)
            {
                ...
                var currentVersion = GetCurrentVersion();
                LogUtil.LogInfo("更新管理器", $"强制结束更新：当前版本={currentVersion}，目标版本={state.TargetVersion}");
                ...
                if (master != null && master.Status != InstanceStatus.Completed)
                {
                    if (currentVersion == state.TargetVersion)
                    {
                        ...
                        master.Version = currentVersion;
                        master.UpdateEndTime = DateTime.UtcNow;
                        LogUtil.LogInfo("更新管理器", "根据版本校验，主控实例已标记为完成");
                    }
                    else
                    {
                        master.Status = InstanceStatus.Failed;
                        master.Error = $"版本不一致：期望 {state.TargetVersion}，实际 {currentVersion}";
                        LogUtil.LogError("更新管理器", $"主控实例被标记为失败：{master.Error}");
                    }
                }
                ...
                state.Message = allSuccess ? "已强制完成更新（全部成功）" : "已强制完成更新（存在错误）";
                state.LastModified = DateTime.UtcNow;
                
                SaveState();
                LogUtil.LogInfo("更新管理器", $"更新会话已强制结束，成功状态={allSuccess}");
            }
        }
    }
    ...
    private static string? ValidateAndSanitizePath(string path)
    {
        try
        {
            ...
            {
                LogUtil.LogError("更新管理器", $"检测到路径遍历尝试：{path}");
                return null;
            }
        ...
        {
            LogUtil.LogError("更新管理器", $"路径校验失败 {path}：{ex.Message}");
            return null;
        }
    }
    ...
    private static async Task<int> FindMasterInstancePortAsync()
    {
        ...
        {
            LogUtil.LogInfo("更新管理器", $"无法通过 API 确定主控端口：{ex.Message}");
        }
