...
public class PokeTradeBotLGPE(PokeTradeHub<PB7> Hub, PokeBotState Config) : PokeRoutineExecutor7LGPE(Config), ICountBot, ITradeBot
{
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        try
        {
            ...
            OnConnectionSuccess();
            Log($"开始 {nameof(PokeTradeBotLGPE)} 主循环。");
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV7b sav, PokeTradeDetail<PB7> poke, CancellationToken token)
    {
        ...
        if (Hub.Config.Legality.UseTradePartnerInfo && !poke.IgnoreAutoOT)
        {
            ...
            {
                Log($"正在基于训练家 OT 应用 AutoOT：{tradeDetails.OT}，TID：{tradeDetails.TID}，SID：{tradeDetails.SID}");
        ...
        if (tradepartnersav.OT != sav.OT)
        {
            ...
            string genderText = gender == 0 ? "男性" : "女性";
            string languageText = GetLanguageName(language);

            Log($"找到连接交换伙伴：{tradepartnersav.OT}，TID7：{tid7}，SID7：{sid7}，性别：{genderText}，语言：{languageText}，游戏：{tradepartnersav.Version}");
            ...
            poke.SendNotification(this, $"已找到伙伴 —— OT：{tradepartnersav.OT}，TID：{tid7}，SID：{sid7}，性别：{genderText}，语言：{languageText}");
        }
        ...
        if (tradepartnersav2.OT != sav.OT)
        {
            ...
            Log($"找到连接交换伙伴：{tradepartnersav2.OT}，TID7：{tid7}，SID7：{sid7}，性别：{genderText}，语言：{languageText}");

            // Save all trainer details in the TradeCodeStorage
            tradeCodeStorage.UpdateTradeDetails(poke.Trainer.ID, tradepartnersav2.OT, int.Parse(tid7), int.Parse(sid7), gender, language);

            // Send notification with trainer details
            poke.SendNotification(this, $"已找到伙伴 —— OT：{tradepartnersav2.OT}，TID：{tid7}，SID：{sid7}，性别：{genderText}，语言：{languageText}");
        }
    ...
    {
        detail.SendNotification(this, "请在盒子中高亮想要克隆的宝可梦，最多一次 6 只！每次高亮后有 5 秒切换到下一只（前 5 秒从现在开始）。若少于 6 只，请停留在同一只宝可梦直至交易开始。");
        ...
        detail.SendNotification(this, "正在退出交易以注入克隆，请使用相同的连接代码重新加入。");
        await ExitTrade(false, token);
        ...
        if (tradepartnersav.OT != sav.OT)
        {
            Log($"找到连接交换伙伴：{tradepartnersav.OT}，TID：{tradepartnersav.DisplayTID}，SID：{tradepartnersav.DisplaySID}，游戏：{tradepartnersav.Version}");
            detail.SendNotification(this, $"已找到连接交换伙伴：{tradepartnersav.OT}，TID：{tradepartnersav.DisplayTID}，SID：{tradepartnersav.DisplaySID}，游戏：{tradepartnersav.Version}");
        }
        if (tradepartnersav2.OT != sav.OT)
        {
            Log($"找到连接交换伙伴：{tradepartnersav2.OT}，TID：{tradepartnersav2.DisplayTID}，SID：{tradepartnersav2.DisplaySID}");
            detail.SendNotification(this, $"已找到连接交换伙伴：{tradepartnersav2.OT}，TID：{tradepartnersav2.DisplayTID}，SID：{tradepartnersav2.DisplaySID}，游戏：{tradepartnersav.Version}");
        }
        foreach (var t in clonelist)
        {
            ...
            detail.SendNotification(this, $"正在发送 {(Species)t.Species}。你有 15 秒时间选择要交换的宝可梦。");
            Log("正在等待交易界面…");

            await Task.Delay(10_000, token).ConfigureAwait(false);
            detail.SendNotification(this, "还剩 5 秒钟，请尽快进入交易界面以免中断交易。");
            await Task.Delay(5_000, token);
    ...
    private async Task<PokeTradeResult> ProcessDumpTradeAsync(PokeTradeDetail<PB7> detail, CancellationToken token)
    {
        ...
        while (quicktime.ElapsedMilliseconds <= 30_000)
        {
            ...
            {
                detail.SendNotification(this, newofferedpbm, "这是你展示的宝可梦。");
    ...
    private async Task EnterLinkCodeLG(PokeTradeDetail<PB7> poke, CancellationToken token)
    {
        if (poke.LGPETradeCode == null || !poke.LGPETradeCode.Any())
        {
            ...
            Log($"使用默认连接代码：{string.Join(", ", poke.LGPETradeCode)}");
        }
        else
        {
            Log($"正在输入连接代码：{string.Join(", ", poke.LGPETradeCode)}");
        }
    ...
    private async Task ExitTrade(bool unexpected, CancellationToken token)
    {
        ...
        {
            attempts++;
            Log($"退出尝试第 {attempts} 次");

            // Check if we've exceeded max attempts
            if (attempts > MAX_ATTEMPTS)
            {
                Log("多次尝试后仍未返回场景，正在重启游戏…");
    ...
    private Task<PB7?> ApplyAutoOT(PB7 toSend, ulong trainerID)
    {
        ...
        if (tradeDetails != null)
        {
            ...
            if (tradelgpe.Valid)
            {
                Log("宝可梦合法，正在应用 AutoOT。");
                return Task.FromResult<PB7?>(cln);
            }
            else
            {
                Log("宝可梦不合法，无法应用 AutoOT。");
