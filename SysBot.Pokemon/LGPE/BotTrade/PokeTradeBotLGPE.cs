...
public class PokeTradeBotLGPE(PokeTradeHub<PB7> Hub, PokeBotState Config) : PokeRoutineExecutor7LGPE(Config), ICountBot, ITradeBot
{
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        try
        {
            ...
            OnConnectionSuccess();
            Log($"开始主 {nameof(PokeTradeBotLGPE)} 循环。");
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV7b sav, PokeTradeDetail<PB7> poke, CancellationToken token)
    {
        ...
        if (Hub.Config.Legality.UseTradePartnerInfo && !poke.IgnoreAutoOT)
        {
            ...
            {
                Log($"使用训练家 OT: {tradeDetails.OT}, TID: {tradeDetails.TID}, SID: {tradeDetails.SID} 应用 AutoOT");
        ...
        if (tradepartnersav.OT != sav.OT)
        {
            ...
            Log($"找到连接交易伙伴: {tradepartnersav.OT}, TID7: {tid7}, SID7: {sid7}, 性别: {genderText}, 语言: {languageText}, 游戏: {tradepartnersav.Version}");
            ...
            poke.SendNotification(this, $"找到伙伴 - OT: {tradepartnersav.OT}, TID: {tid7}, SID: {sid7}, 性别: {genderText}, 语言: {languageText}");
        }
        ...
        if (tradepartnersav2.OT != sav.OT)
        {
            ...
            Log($"找到连接交易伙伴: {tradepartnersav2.OT}, TID7: {tid7}, SID7: {sid7}, 性别: {genderText}, 语言: {languageText}");

            // Save all trainer details in the TradeCodeStorage
            tradeCodeStorage.UpdateTradeDetails(poke.Trainer.ID, tradepartnersav2.OT, int.Parse(tid7), int.Parse(sid7), gender, language);

            // Send notification with trainer details
            poke.SendNotification(this, $"找到伙伴 - OT: {tradepartnersav2.OT}, TID: {tid7}, SID: {sid7}, 性别: {genderText}, 语言: {languageText}");
        }
        ...
        poke.SendNotification(this, "你有 15 秒钟来选择你要交易的宝可梦");
        Log("正在等待交易界面...");
    ...
    {
        detail.SendNotification(this, "请在盒子中选择你想要克隆的宝可梦，一次最多 6 只！你有 5 秒钟时间在高亮之间移动到下一只宝可梦（第一个 5 秒现在开始！）。如果你想克隆少于 6 只，请停留在同一只宝可梦上直到交易开始。");
        ...
        detail.SendNotification(this, $"你已将 {(Species)offeredpbmc.Species} 添加到克隆列表");
        ...
        detail.SendNotification(this, "正在退出交易以注入克隆，请使用相同的连接密码重新连接。");
        await ExitTrade(false, token);
        ...
        if (tradepartnersav.OT != sav.OT)
        {
            Log($"找到连接交易伙伴: {tradepartnersav.OT}, TID: {tradepartnersav.DisplayTID}, SID: {tradepartnersav.DisplaySID}, 游戏: {tradepartnersav.Version}");
            detail.SendNotification(this, $"找到连接交易伙伴: {tradepartnersav.OT}, TID: {tradepartnersav.DisplayTID}, SID: {tradepartnersav.DisplaySID}, 游戏: {tradepartnersav.Version}");
        }
        if (tradepartnersav2.OT != sav.OT)
        {
            Log($"找到连接交易伙伴: {tradepartnersav2.OT}, TID: {tradepartnersav2.DisplayTID}, SID: {tradepartnersav2.DisplaySID}");
            detail.SendNotification(this, $"找到连接交易伙伴: {tradepartnersav2.OT}, TID: {tradepartnersav2.DisplayTID}, SID: {tradepartnersav2.DisplaySID}, 游戏: {tradepartnersav.Version}");
        }
        foreach (var t in clonelist)
        {
            ...
            detail.SendNotification(this, $"正在发送 {(Species)t.Species}。你有 15 秒钟来选择你要交易的宝可梦");
            Log("正在等待交易界面...");

            await Task.Delay(10_000, token).ConfigureAwait(false);
            detail.SendNotification(this, "你还有 5 秒钟进入交易界面，以免中断交易");
            await Task.Delay(5_000, token);
    ...
    {
        detail.SendNotification(this, "请在盒子中选择宝可梦，你有 30 秒钟时间");
    ...
    private async Task EnterLinkCodeLG(PokeTradeDetail<PB7> poke, CancellationToken token)
    {
        if (poke.LGPETradeCode == null || !poke.LGPETradeCode.Any())
        {
            ...
            Log($"使用默认交易密码: {string.Join(", ", poke.LGPETradeCode)}");
        }
        else
        {
            Log($"正在输入交易密码: {string.Join(", ", poke.LGPETradeCode)}");
        }
    ...
    private Task<PB7?> ApplyAutoOT(PB7 toSend, ulong trainerID)
    {
        ...
        if (tradeDetails != null)
        {
            ...
            if (tradeDetails.Language.HasValue)
            {
                ...
                Log($"正在设置宝可梦语言为: {GetLanguageName(tradeDetails.Language.Value)}");
            }
            else
            {
    ...
    private static void ClearOTTrash(PB7 pokemon, TradeCodeStorage.TradeCodeDetails? tradeDetails)
    {
        ...
        {
            LogUtil.LogInfo("AutoOT", "交易详情或 OT 为空。跳过 ClearOTTrash。");
            return;
        }
