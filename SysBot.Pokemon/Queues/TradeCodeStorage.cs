...
public class TradeCodeStorage
{
    ...
    private void SaveToFile()
    {
        ...
        {
            LogUtil.LogInfo("TradeCodeStorage", $"保存交易代码到文件时出错：{ex.Message}");
        }
        catch (UnauthorizedAccessException ex)
        {
            LogUtil.LogInfo("TradeCodeStorage", $"保存交易代码到文件时被拒绝访问：{ex.Message}");
        }
        catch (Exception ex)
        {
            LogUtil.LogInfo("TradeCodeStorage", $"保存交易代码到文件时发生异常：{ex.Message}");
        }
    }
