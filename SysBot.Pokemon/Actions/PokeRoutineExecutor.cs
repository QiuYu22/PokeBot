    ...
    where T : PKM, new()
{
        ...
        TradeAbuseSettings AbuseSettings, CancellationToken token)
    {
        ...
        if (previous != null)
        {
            ...
            Log($"上次与 {user.TrainerName} 交易是 {delta.TotalMinutes:F1} 分钟前（OT: {TrainerName}）。");
            ...
            {
                Log($"发现 {user.TrainerName} 无视 {cd} 分钟的交易冷却时间。上次遇到是 {delta.TotalMinutes:F1} 分钟前。");
                return PokeTradeResult.SuspiciousActivity;
            }
            ...
            if (isDistribution && previous.NetworkID == TrainerNID && previous.RemoteID != user.ID)
            {
                ...
                {
                    quit = true;
                    Log($"发现 {user.TrainerName} 使用多个账号。\n之前与 {previous.Name} ({previous.RemoteID}) 在 {delta.TotalMinutes:F1} 分钟前交易过，OT: {TrainerName}。");
                }
            }
        }
