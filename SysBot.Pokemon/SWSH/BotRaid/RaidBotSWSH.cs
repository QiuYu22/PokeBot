...
public class RaidBotSWSH(PokeBotState Config, PokeTradeHub<PK8> Hub) : PokeRoutineExecutor8SWSH(Config), ICountBot
{
    ...
    private async Task<bool> ConfirmPlayerReady(uint player, CancellationToken token)
    {
        ...
        if (Settings.EchoPartyReady)
        {
            ...
            var gender = data[0] == 0 ? "（♂）" : (data[0] == 1 ? "（♀）" : "");

            EchoUtil.Echo($"玩家 {player + 1} 已准备，使用 {shiny}{(Species)dexno}{altformstr}{gender}！");
        }
    ...
    private async Task HostRaidAsync(int code, CancellationToken token)
    {
        ...
        if (addFriends && !string.IsNullOrEmpty(Settings.FriendCode))
            EchoUtil.Echo($"如需加入，请向好友代码 **{Settings.FriendCode}** 发送请求！完成本次团战后将添加好友。");
        ...
        var linkcodemsg = code < 0 ? "无连接代码" : $"连接代码 **{code:0000 0000}**";
        ...
        EchoUtil.Echo($"{raiddescmsg} 的团战房间已开放（{linkcodemsg}）。");
