...
public class PokeTradeBotSWSH(PokeTradeHub<PK8> hub, PokeBotState config) : PokeRoutineExecutor8SWSH(config), ICountBot, ITradeBot
{
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        try
        {
            ...
            OnConnectionSuccess();
            Log($"正在启动 {nameof(PokeTradeBotSWSH)} 主循环。");
    ...
    private async Task<PokeTradeResult> PerformBatchTrade(SAV8SWSH sav, PokeTradeDetail<PK8> poke, CancellationToken token)
    {
        ...
        Log($"已找到连线交换对象：{trainerName}-{trainerTID}（ID：{trainerNID}）");
        poke.SendNotification(this, $"已找到连线交换对象：{trainerName}。**TID**：{trainerTID} **SID**：{trainerSID}");
        ...
        for (int i = 0; i < totalBatchTrades; i++)
        {
            ...
            if (currentTradeIndex > 0)
            {
                ...
                if (await IsOnOverworld(OverworldOffset, token).ConfigureAwait(false))
                {
                    poke.SendNotification(this, "连接丢失，对方可能已断线。");
                    SendCollectedPokemonAndCleanup();
            ...
            if (!await IsInBox(token).ConfigureAwait(false))
            {
                ...
                if (!await IsInBox(token).ConfigureAwait(false))
                {
                    poke.SendNotification(this, $"在第 {completedTrades + 1}/{totalBatchTrades} 次交易后无法进入盒子，正在取消剩余交易。");
                    SendCollectedPokemonAndCleanup();
                    await ExitTrade(true, token).ConfigureAwait(false);
            ...
            if (offered == null)
            {
                poke.SendNotification(this, $"第 {currentTradeIndex + 1}/{totalBatchTrades} 次交易未提供宝可梦，正在取消剩余交易。");
                SendCollectedPokemonAndCleanup();
                await ExitTrade(false, token).ConfigureAwait(false);
            ...
            {
                poke.SendNotification(this, $"第 {currentTradeIndex + 1}/{totalBatchTrades} 次交易检查失败，正在取消剩余交易。");
                SendCollectedPokemonAndCleanup();
                await ExitTrade(false, token).ConfigureAwait(false);
                return update.check;
            }
            ...
            else
            {
                ...
                poke.SendNotification(this, $"第 {completedTrades} 次交易完成！正在准备第 {completedTrades + 1}/{totalBatchTrades} 次交易...");
            }
        }
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8SWSH sav, PokeTradeDetail<PK8> poke, CancellationToken token)
    {
        ...
        Log($"已找到连线交换对象：{trainerName}。**TID**：{trainerTID}  **SID**：{trainerSID}。等待对方提供宝可梦…");
...
#pragma warning restore CS8604 // Possible null reference argument.
        ...
        poke.SendNotification(this, $"已找到连线交换对象：{trainerName}。**TID**：{trainerTID}  **SID**：{trainerSID}。等待对方提供宝可梦…");
        ...
        Log($"交易完成。收到用户提供的 {GameInfo.GetStrings("zh").Species[received.Species]}，送出 {GameInfo.GetStrings("zh").Species[toSend.Species]}。");
        poke.TradeFinished(this, received);
    ...
    private async Task<PokeTradeResult> PerformSurpriseTrade(SAV8SWSH sav, PK8 pkm, CancellationToken token)
    {
        ...
        Log($"已找到惊喜交换伙伴：{TrainerName}-{TrainerTID}，宝可梦：{(Species)SurprisePoke.Species}");
...
#pragma warning restore 4014
    ...
    private void ReplyWithSeedCheckResults(PokeTradeDetail<PK8> detail, PK8 result)
    {
        ...
        {
            Log("该宝可梦已经是闪光状态！");
            detail.SendNotification(this, "该宝可梦已经是闪光，未执行团战种子计算。");
    ...
    private async Task<(PK8 toSend, PokeTradeResult check)> HandleFixOT(SAV8SWSH sav, PokeTradeDetail<PK8> poke, PK8 offered, PartnerDataHolder partner, CancellationToken token)
    {
        ...
        await SetBoxPokemon(clone, 0, 0, token, sav).ConfigureAwait(false);
        await Click(A, 0_500, token).ConfigureAwait(false);
        poke.SendNotification(this, "请立即确认交易！");
    ...
    private async Task<PK8> ApplyAutoOT(PK8 toSend, string trainerName, SAV8SWSH sav, CancellationToken token)
    {
        ...
        if (toSend.Version == GameVersion.GO)
        {
            ...
            if (toSend is IHomeTrack { HasTracker: true })
            {
                ...
                Log("已为来自 GO 且带有 HOME 追踪器的宝可梦仅应用 OT 名称。");
            ...
            else
            {
                ...
                Log("已为来自 GO 且无 HOME 追踪器的宝可梦应用 OT、TID 与 SID。");
                await SetBoxPokemon(goClone, 0, 0, token, sav).ConfigureAwait(false);
                return goClone;
            }
        }
