...
public class ShowdownTranslator<T> where T : PKM
{
    ...
    public static bool IsPS(string str)
    {
        ...
        return GameStringsEn.Species
            .Skip(1)
            .Any(name => !string.IsNullOrEmpty(name) && str.Contains(name, StringComparison.OrdinalIgnoreCase));
    }

    public static string TranslateIfChinese(string content)
    {
        ...
        var translated = Chinese2Showdown(content);
        return string.IsNullOrWhiteSpace(translated) ? content : translated;
    }

    private static bool ContainsCjk(ReadOnlySpan<char> span)
    {
