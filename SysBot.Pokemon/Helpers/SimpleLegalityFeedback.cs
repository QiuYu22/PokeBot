...
public static class SimpleLegalityFeedback
{
    public static string GetLegalityReport(PKM pkm, LegalityAnalysis la, string speciesName)
    {
        ...
        // Header
        sb.AppendLine($"**{speciesName} 合法性检测**");
        ...
        if (!la.Valid)
        {
            ...
            {
                sb.AppendLine("\n**招式提示：**");
                sb.AppendLine("  • 检查招式是否存在于目标世代");
                sb.AppendLine("  • 确认招式组合是否合法");
                sb.AppendLine("  • 部分招式仅限活动获取");
            }
        }
        else
        {
            sb.AppendLine($"\n✨ 你的 {speciesName} 通过了全部合法性检测！");
            if (la.EncounterOriginal != null)
            {
                sb.AppendLine($"遭遇信息：{la.EncounterOriginal.LongName}");
            }
        }
