...
#pragma warning restore CS0414
    ...
    public override async Task MainLoop(CancellationToken token)
    {
        try
        {
            ...
            Log($"开始主循环 {nameof(PokeTradeBotSV)}。");
    ...
    private async Task<PK9> ApplyAutoOT(PK9 toSend, TradeMyStatus tradePartner, SAV9SV sav, CancellationToken token)
    {
        ...
        {
            Log("检测到 HOME 追踪码，无法应用 AutoOT。");
            return toSend;
        }

        if (toSend.Generation != toSend.Format)
        {
            Log("无法应用伙伴信息：当前训练家不可来自不同世代。");
            return toSend;
        }
    ...
    private async Task<bool> ConnectToOnline(PokeTradeHubConfig config, CancellationToken token)
    {
        ...
        while (true) // Loop until a successful connection is made or the task is canceled
        {
            ...
            try
            {
                ...
                attemptCount++;
                Log($"第 {attemptCount} 次尝试（共 {maxAttempt} 次）：正在联网…");
            ...
            {
                Log($"连接尝试出现异常：{ex.Message}");

                if (attemptCount >= maxAttempt)
                {
                    Log($"因异常导致 {maxAttempt} 次连接失败，等待 {waitTime} 分钟后重试。");
                    await Task.Delay(TimeSpan.FromMinutes(waitTime), token).ConfigureAwait(false);
                    Log("正在尝试重新打开游戏。");
                    await ReOpenGame(Hub.Config, token).ConfigureAwait(false);
                    attemptCount = 0;
    ...
    private async Task DoTrades(SAV9SV sav, CancellationToken token)
    {
        ...
        Log($"开始下一笔 {type}{tradetype} 交易，正在获取数据…");
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleClone(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, byte[] oldEC, CancellationToken token)
    {
        ...
        if (!la.Valid)
        {
            ...
            Log(report);
            poke.SendNotification(this, "该宝可梦未通过 PKHeX 合法性检测，无法克隆，正在退出交易。");
        ...
        poke.SendNotification(this, $"**已克隆你的 {GameInfo.GetStrings("zh-Hans").Species[clone.Species]}！**\n请按 B 取消交换宝可梦，并交易一只你不需要的宝可梦给我。");
        Log($"已克隆 {GameInfo.GetStrings("zh-Hans").Species[clone.Species]}，等待用户更换宝可梦…");
        ...
        if (!partnerFound)
        {
            poke.SendNotification(this, "**请立刻更换，否则我就离开！**");
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleFixOT(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, PartnerDataHolder partnerID, CancellationToken token)
    {
        ...
        if (!laInit.Valid)
        {
            Log($"FixOT 请求检测到 {name} 交出的非法宝可梦：{(Species)offered.Species}");
            var report = laInit.Report();
            Log(laInit.Report());
            poke.SendNotification(this, $"**检测到展示的宝可梦不合法，正在尝试重新生成…**\n\n```{report}```");
            if (DumpSetting.Dump)
                DumpPokemon(DumpSetting.DumpFolder, "hacked", offered);
        }
        ...
        if (!la.Valid)
        {
            poke.SendNotification(this, "该宝可梦未通过 PKHeX 合法性检测，无法修复，正在退出交易。");
            return (clone, PokeTradeResult.IllegalTrade);
        }

        TradeExtensions<PK9>.HasAdName(offered, out string detectedAd);
        poke.SendNotification(this, $"{(!laInit.Valid ? "**已合法化**" : "**已修复昵称/OT**")} {(Species)clone.Species}（检测到广告：{detectedAd}）！请确认交易！");
        ...
        {
            // They get one more chance.
            poke.SendNotification(this, "**请重新放出最初展示的宝可梦，否则我将离开！**");
        ...
        if (changed)
        {
            poke.SendNotification(this, "对方更换了宝可梦且未换回，正在退出交易。");
            Log("交易伙伴不愿放出带广告的宝可梦。");
            return (offered, PokeTradeResult.TrainerTooSlow);
        }
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleRandomLedy(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, PK9 toSend, PartnerDataHolder partner, CancellationToken token)
    {
        ...
        var nickname = offered.IsNicknamed ? $"（昵称：\"{offered.Nickname}\"）" : string.Empty;
            poke.SendNotification(this, $"未找到与 {GameInfo.GetStrings("zh-Hans").Species[offered.Species]}{nickname} 匹配的请求。");
    ...
    private async Task<PokeTradeResult> PerformBatchTrade(SAV9SV sav, PokeTradeDetail<PK9> poke, CancellationToken token)
    {
        ...
        void SendCollectedPokemonAndCleanup()
        {
            ...
            if (allReceived.Count > 0)
            {
                ...
                for (int j = 0; j < allReceived.Count; j++)
                {
                    ...
                    Log($"  - 归还：{speciesName}（校验值：{pokemon.Checksum:X8}）");
        ...
        for (int i = 0; i < totalBatchTrades; i++)
        {
            ...
            if (currentTradeIndex == 0)
            {
                ...
                while (!await IsInBox(PortalOffset, token).ConfigureAwait(false))
                {
                    ...
                    if (++cnt > 20)
                    {
                        ...
                        poke.SendNotification(this, "未能进入交易盒，正在取消批量交易。");
                ...
                RecordUtil<PokeTradeBotSV>.Record($"启动中\t{trainerNID:X16}\t{tradePartner.TrainerName}\t{poke.Trainer.TrainerName}\t{poke.Trainer.ID}\t{poke.ID}\t{toSend.EncryptionConstant:X8}");
                ...
                if (shouldUpdateOT || shouldUpdateTID || shouldUpdateSID)
                {
                    ...
                    {
                        Log("OT/TID/SID 存在空值，跳过 自动更新交易详情。");
                    }
                }
                ...
                if (!tradeOffered)
                {
                    poke.SendNotification(this, "交易伙伴响应超时，正在取消批量交易。");
                    SendCollectedPokemonAndCleanup();
                    await ExitTradeToPortal(false, token).ConfigureAwait(false);
                    return PokeTradeResult.TrainerTooSlow;
                }
                ...
                poke.SendNotification(this, $"已找到链接交换伙伴：{tradePartner.TrainerName}，**TID**：{tradePartner.TID7} **SID**：{tradePartner.SID7}");
            ...
            {
                Log("交易结束，因为对方没有提供有效宝可梦。");
                poke.SendNotification(this, $"第 {currentTradeIndex + 1}/{totalBatchTrades} 笔交易提供的宝可梦无效，正在取消剩余交易。");
                SendCollectedPokemonAndCleanup();
                await ExitTradeToPortal(false, token).ConfigureAwait(false);
                return PokeTradeResult.TrainerTooSlow;
            }
            ...
            {
                poke.SendNotification(this, $"第 {currentTradeIndex + 1}/{totalBatchTrades} 笔交易更新检查失败，正在取消剩余交易。");
                SendCollectedPokemonAndCleanup();
                await ExitTradeToPortal(false, token).ConfigureAwait(false);
                return update;
            }
            ...
            Log($"已将收到的宝可梦 {received.Species}（校验值：{received.Checksum:X8}）记录到训练家 {originalTrainerID} 的批量追踪（第 {completedTrades}/{totalBatchTrades} 笔交易）");

            if (completedTrades == totalBatchTrades)
            {
                // Get all collected Pokemon before cleaning anything up
                var allReceived = BatchTracker.GetReceivedPokemon(originalTrainerID);
                Log($"批量交易完成，训练家 {originalTrainerID} 的追踪中共有 {allReceived.Count} 只宝可梦");
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV9SV sav, PokeTradeDetail<PK9> poke, CancellationToken token)
    {
        ...
        if (poke.IsCanceled)
        {
            Log($"{poke.Trainer.TrainerName} 的交易已被用户取消。");
    ...
    private async Task<PokeTradeResult> PerformNonBatchTrade(SAV9SV sav, PokeTradeDetail<PK9> poke, CancellationToken token)
    {
        ...
        RecordUtil<PokeTradeBotSV>.Record($"启动中\t{trainerNID:X16}\t{tradePartner.TrainerName}\t{poke.Trainer.TrainerName}\t{poke.Trainer.ID}\t{poke.ID}\t{toSend.EncryptionConstant:X8}");
        Log($"找到连接交换伙伴：{tradePartner.TrainerName}-{tradePartner.TID7}（ID：{trainerNID}）");
        poke.SendNotification(this, $"已找到连接交换伙伴：{tradePartner.TrainerName}。TID：{tradePartner.TID7} SID：{tradePartner.SID7}，等待对方交换…");
        ...
        if (shouldUpdateOT || shouldUpdateTID || shouldUpdateSID)
        {
            ...
            {
                Log("OT/TID/SID 存在空值，跳过自动更新交易详情。");
            }
        }
        ...
        if (poke.Type == PokeTradeType.Seed && itemReq == SpecialTradeType.None)
        {
            ...
            poke.SendNotification(this, "未检测到携带物或有效请求，正在取消本次交易。");
        ...
        if (update != PokeTradeResult.Success)
        {
            ...
            {
                poke.SendNotification(this, "请求不合法，请尝试更换宝可梦或请求内容。");
            }
