...
public abstract class PokeRoutineExecutor9SV : PokeRoutineExecutor<PK9>
{
    ...
    public async Task<SAV9SV> IdentifyTrainer(CancellationToken token)
    {
        ...
        if (title is not (ScarletID or VioletID))
            throw new Exception($"{title} 不是有效的 SV 标题，是否选择了正确的模式？");

        // Verify the game version.
        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(SVGameVersion))
            throw new Exception($"不支持的游戏版本，应为 {SVGameVersion}，当前为 {game_version}。");
        ...
        if (!IsValidTrainerData())
        {
            ...
            throw new Exception("训练家数据无效，请参阅 SysBot.NET Wiki（https://github.com/kwsch/SysBot.NET/wiki/Troubleshooting）获取更多信息。");
        }

        if (await GetTextSpeed(token).ConfigureAwait(false) < TextSpeedOption.Fast)
            throw new Exception("文本速度需设置为“快速”，请调整后继续操作。");
