...
public abstract class PokeRoutineExecutor8BS : PokeRoutineExecutor<PB8>
{
    ...
    public async Task<SAV8BS> IdentifyTrainer(CancellationToken token)
    {
        ...
        Offsets = title switch
        {
            ...
            _ => throw new Exception($"{title} 不是有效的 Pokémon 晶灿钻石/晶亮珍珠游戏标题。请检查运行模式是否正确。"),
        };

        // Verify the game version.
        var game_version = await SwitchConnection.GetGameInfo("version", token).ConfigureAwait(false);
        if (!game_version.SequenceEqual(BSGameVersion))
            throw new Exception($"当前游戏版本不受支持。期望版本为 {BSGameVersion}，检测到的版本为 {game_version}。");
        ...
        if (!IsValidTrainerData())
        {
            ...
            throw new Exception("更多信息请参考 SysBot.NET Wiki： https://github.com/kwsch/SysBot.NET/wiki/Troubleshooting");
        }

        if (await GetTextSpeed(token).ConfigureAwait(false) < TextSpeedOption.Fast)
            throw new Exception("文本速度必须设置为“快速”，否则无法正确运行。");
