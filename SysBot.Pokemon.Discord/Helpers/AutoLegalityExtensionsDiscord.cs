...
public static class AutoLegalityExtensionsDiscord
{
    public static async Task ReplyWithLegalizedSetAsync(this ISocketMessageChannel channel, ITrainerInfo sav, ShowdownSet set)
    {
        if (set.Species <= 0)
        {
            await channel.SendMessageAsync("哎呀！我无法理解您的消息！如果您想转换某些内容，请仔细检查您粘贴的内容！").ConfigureAwait(false);
            return;
        }
        ...
        try
        {
            ...
            if (!la.Valid)
            {
                var reason = result == "Timeout" ? $"该 {spec} 配置生成时间过长。" : result == "VersionMismatch" ? "请求被拒绝: PKHeX 和 Auto-Legality Mod 版本不匹配。" : $"无法从该配置创建 {spec}。";
                var imsg = $"哎呀！{reason}";
                if (result == "Failed")
