...
public static class Helpers<T> where T : PKM, new()
{
    ...
    public static Task<ProcessedPokemonResult<T>> ProcessShowdownSetAsync(string content, bool ignoreAutoOT = false)
    {
        ...
        if (!ShowdownParsing.TryParseAnyLanguage(content, out ShowdownSet? set) || set == null || set.Species == 0)
        {
            ...
            {
                Error = "无法解析 Showdown 配置。无法识别宝可梦种类。",
                ShowdownSet = set
            });
        }
        ...
        if (pkm is PB7)
        {
            ...
            if (pkm.Species == (int)Species.Mew && pkm.IsShiny)
            {
                return Task.FromResult(new ProcessedPokemonResult<T>
                {
                    Error = "梦幻在 LGPE 中**不能**是闪光的。PoGo 梦幻无法传输，精灵球 Plus 梦幻已锁闪。",
                    ShowdownSet = set
                });
            }
        }
        ...
        if (Info.Hub.Config.Trade.TradeConfiguration.EnableSpamCheck)
        {
            if (TradeExtensions<T>.HasAdName(pk, out string ad))
            {
                return Task.FromResult(new ProcessedPokemonResult<T>
                {
                    Error = "在宝可梦名称或训练家名称中检测到广告名称，这是不允许的。",
                    ShowdownSet = set
                });
            }
        }
    ...
    public static async Task SendTradeErrorEmbedAsync(SocketCommandContext context, ProcessedPokemonResult<T> result)
    {
        ...
        string messageContent = $"{userMention}，这是您请求的报告:";
    ...
    public static async Task<T?> ProcessTradeAttachmentAsync(SocketCommandContext context)
    {
        ...
        if (pk == null)
        {
            _ = await context.Channel.SendMessageAsync("提供的附件与此模块不兼容！").ConfigureAwait(false);
            return null;
        }
        ...
        bool isNonNative = false)
    {
        ...
        {
            var reply = await context.Channel.SendMessageAsync("提供的宝可梦内容禁止交易！").ConfigureAwait(false);
        ...
        {
            var itemName = pk.HeldItem > 0 ? GameInfo.GetStrings("en").Item[pk.HeldItem] : "(无)";
            var reply = await context.Channel.SendMessageAsync($"交易被阻止：携带道具 '{itemName}' 无法进行交易。").ConfigureAwait(false);
            await Task.Delay(6000).ConfigureAwait(false);
            await reply.DeleteAsync().ConfigureAwait(false);
            return;
        }
        ...
        if (!la.Valid)
        {
            ...
            if (pk?.IsEgg == true)
            {
                ...
                responseMessage = $"{speciesName} 蛋的 Showdown 配置无效。请检查您的信息后重试。\n\n合法性报告:\n```\n{la.Report()}\n```";
            }
            else
            {
                string speciesName = SpeciesName.GetSpeciesName(pk!.Species, (int)LanguageID.English);
                responseMessage = $"{speciesName} 附件不合法，无法进行交易！\n\n合法性报告:\n```\n{la.Report()}\n```";
            }
        ...
        {
            string speciesName = SpeciesName.GetSpeciesName(pk!.Species, (int)LanguageID.English);
            _ = await context.Channel.SendMessageAsync($"此 **{speciesName}** 不是此游戏原生的，无法交易！请使用正确的机器人交易，然后传输到 HOME。").ConfigureAwait(false);
            return;
        }
        ...
        {
            string speciesName = SpeciesName.GetSpeciesName(pk.Species, (int)LanguageID.English);
            _ = await context.Channel.SendMessageAsync($"此 {speciesName} 文件被 HOME 追踪，无法进行交易！").ConfigureAwait(false);
            return;
        }
