...
public static class ListHelpers<T> where T : PKM, new()
{
        ...
        string commandPrefix, string args)
    {
        ...
        {
            await Helpers<T>.ReplyAndDeleteAsync(context, "此机器人未设置此功能。", 2);
            return;
        }
        ...
        {
            var replyMessage = await context.Channel.SendMessageAsync($"未找到匹配过滤器 '{filter}' 的{itemType}。");
        ...
        var embed = new EmbedBuilder()
            .WithTitle($"可用的{char.ToUpper(itemType[0]) + itemType[1..]} - 过滤器: '{filter}'")
        ...
        foreach (var item in pageItems)
        {
            ...
            embed.AddField($"{index}. {item}", $"使用 `{botPrefix}{commandPrefix} {index}` 请求此{itemType.TrimEnd('s')}。");
        }
    ...
    public static async Task SendDMOrReplyAsync(SocketCommandContext context, Embed embed)
    {
        ...
        if (context.User is IUser user)
        {
            try
            {
                ...
                replyMessage = await context.Channel.SendMessageAsync($"{context.User.Mention}，我已通过私信向您发送了列表。");
            }
            catch (HttpException ex) when (ex.HttpCode == HttpStatusCode.Forbidden)
            {
                replyMessage = await context.Channel.SendMessageAsync($"{context.User.Mention}，我无法向您发送私信。请检查您的**服务器隐私设置**。");
            }
        }
        else
        {
            replyMessage = await context.Channel.SendMessageAsync("**错误**: 无法发送私信。请检查您的**服务器隐私设置**。");
        }
        ...
        string itemType, string listCommand)
    {
        ...
        try
        {
            ...
            {
                await Helpers<T>.ReplyAndDeleteAsync(context,
                    $"无效的{itemType}索引。请使用来自 `.{listCommand}` 命令的有效数字。", 2);
                return;
            }
            ...
            await context.Channel.SendMessageAsync($"{char.ToUpper(itemType[0]) + itemType[1..]}请求已添加到队列。").ConfigureAwait(false);
