...
public static class ListHelpers<T> where T : PKM, new()
{
        ...
        string commandPrefix, string args)
    {
        ...
        {
            await Helpers<T>.ReplyAndDeleteAsync(context, "此功能未在机器人上启用。", 2);
            return;
        }
        ...
        {
            var replyMessage = await context.Channel.SendMessageAsync($"筛选条件“{filter}”未找到任何 {itemType}。");
        ...
        var embed = new EmbedBuilder()
            .WithTitle($"{char.ToUpper(itemType[0]) + itemType[1..]} 列表 - 筛选：“{filter}”")
    ...
    public static async Task SendDMOrReplyAsync(SocketCommandContext context, Embed embed)
    {
        ...
        if (context.User is IUser user)
        {
            ...
            {
                replyMessage = await context.Channel.SendMessageAsync($"{context.User.Mention}，无法通过私信发送列表，请检查你的**服务器隐私设置**。");
            }
        }
        else
        {
        ...
        string itemType, string listCommand)
    {
        ...
        try
        {
            ...
            {
                await Helpers<T>.ReplyAndDeleteAsync(context,
                    $"{itemType} 序号无效，请使用 `.{listCommand}` 命令中提供的编号。", 2);
                return;
            }
