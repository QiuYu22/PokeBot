...
public static class QueueHelper<T> where T : PKM, new()
{
    ...
    public static async Task AddToQueueAsync(SocketCommandContext context, int code, string trainer, RequestSignificance sig, T trade, PokeRoutineType routine, PokeTradeType type, SocketUser trader, bool isBatchTrade = false, int batchTradeNumber = 1, int totalBatchTrades = 1, bool isHiddenTrade = false, bool isMysteryEgg = false, List<Pictocodes>? lgcode = null, bool ignoreAutoOT = false, bool setEdited = false, bool isNonNative = false)
    {
        ...
        {
            await context.Channel.SendMessageAsync("交易密码必须介于 00000000-99999999 之间！").ConfigureAwait(false);
            return;
        }
        ...
        try
        {
            ...
            if (!isBatchTrade)
            {
                if (trade is PB7 && lgcode != null)
                {
                    ...
                    await trader.SendFileAsync(thefile, "这是你的交易密码。", embed: lgcodeembed).ConfigureAwait(false);
                }
                else
                {
        ...
        List<Pictocodes>? lgcode = null, bool ignoreAutoOT = false, bool setEdited = false, bool isNonNative = false)
    {
        ...
        {
            await context.Channel.SendMessageAsync($"{trader.Mention} - 你已经在队列中，请耐心等待！").ConfigureAwait(false);
        ...
        if (added == QueueResultAdd.QueueFull)
        {
            ...
            var embed = new EmbedBuilder()
                ...
                .WithTitle("🚫 队列已满")
                .WithDescription($"当前队列已满（{maxCount}/{maxCount}）。请稍后再试，等待空位释放。")
                .WithFooter("队列会在完成交易后自动开放")
        ...
        if (added == QueueResultAdd.NotAllowedItem)
        {
            ...
            await context.Channel.SendMessageAsync($"{trader.Mention} - 已阻止该交易：携带物品「{itemName}」无法在 PLZA 中交易。").ConfigureAwait(false);
            return new TradeQueueResult(false);
        }
        ...
        try
        {
            ...
            var etaMessage = $"预计耗时：{baseEta:F1} 分钟。";
            ...
            if (pk is IHomeTrack homeTrack)
            {
                if (homeTrack.HasTracker && isNonNative)
                {
                    ...
                    embedBuilder.AddField("**__提示__**：**该宝可梦为非原生且拥有 HOME 追踪。**", "*未应用 AutoOT。*");
                }
                ...
                {
                    embedBuilder.Footer.IconUrl = "https://raw.githubusercontent.com/hexbyt3/sprites/main/exclamation.gif";
                    embedBuilder.AddField("**__提示__**：**该宝可梦为非原生。**", "*无法进入 HOME，未应用 AutoOT。*");
                }
            }
            else if (isNonNative)
            {
            ...
            if (!isHiddenTrade && SysCord<T>.Runner.Config.Trade.TradeEmbedSettings.UseEmbeds)
            {
                ...
                if (embed == null)
                {
                    Console.WriteLine("错误：嵌入对象为空。");
                    await context.Channel.SendMessageAsync("准备交易详情时出现错误。").ConfigureAwait(false);
                    return new TradeQueueResult(false);
                }
            ...
            {
                var message = $"{trader.Mention} - 已加入连线交换队列。当前队列位置：{position.Position}。将收到：{embedData.SpeciesName}。\n{etaMessage}";
    ...
    public static async Task AddBatchContainerToQueueAsync(SocketCommandContext context, int code, string trainer, T firstTrade, List<T> allTrades, RequestSignificance sig, SocketUser trader, int totalBatchTrades)
    {
        ...
        if (added == QueueResultAdd.QueueFull)
        {
            ...
            var embed = new EmbedBuilder()
                .WithColor(DiscordColor.Red)
                .WithTitle("🚫 队列已满")
                .WithDescription($"当前队列已满（{maxCount}/{maxCount}）。请等待有空位后再试。")
                .WithFooter("队列会在完成交易后自动开放")
                .WithTimestamp(DateTimeOffset.Now)
                .Build();
        ...
        await context.Channel.SendMessageAsync($"{trader.Mention} - 已提交 {totalBatchTrades} 只宝可梦的批量交易，当前队列位置：{position.Position}。预计耗时 {baseEta:F1} 分钟。").ConfigureAwait(false);
...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        else
        {
            ...
            if (!ballImageLoaded)
            {
                Console.WriteLine($"无法加载球图标：{ballImgUrl}");
            }
        }
    ...
    private static async Task<(System.Drawing.Image?, bool)> OverlayBallOnSpecies(string speciesImageUrl, string ballImageUrl)
    {
        ...
        if (ballImage == null)
        {
            Console.WriteLine($"无法加载球图标：{ballImageUrl}");
#pragma warning disable CA1416 // Validate platform compatibility
...
#pragma warning restore CA1416 // Validate platform compatibility
    ...
    private static async Task<System.Drawing.Image> OverlaySpeciesOnEgg(string eggImageUrl, string speciesImageUrl)
    {
        ...
        {
            throw new InvalidOperationException("无法加载蛋或宝可梦图像。");
        }
...
#pragma warning restore CA1416 // Validate platform compatibility
    ...
    private static async Task<System.Drawing.Image?> LoadImageFromUrl(string url)
    {
        ...
        {
            Console.WriteLine($"从 {url} 加载图像失败，状态码：{response.StatusCode}");
            return null;
        }
        ...
        {
            Console.WriteLine($"从 {url} 收到的数据为空或无内容。");
            return null;
        }
...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        {
            Console.WriteLine($"根据流创建图像失败。URL：{url}，异常：{ex}");
            return null;
        }
    }
    ...
    private static async Task HandleDiscordExceptionAsync(SocketCommandContext context, SocketUser trader, HttpException ex)
    {
        ...
        switch (ex.DiscordCode)
        {
            ...
            case DiscordErrorCode.CannotSendMessageToUser:
                {
                    message = context.User == trader ? "必须开启私信功能才能加入队列！" : "该用户需要开启私信功能才能加入队列！";
                }
                break;
