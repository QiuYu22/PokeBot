...
public class RemoteControlModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task ClickAsync(SwitchButton b)
    {
        ...
        if (bot == null)
        {
            await ReplyAsync($"没有可执行该指令的机器人：{b}").ConfigureAwait(false);
            return;
        }
    ...
    private async Task SetScreenForAllBots(bool on)
    {
        ...
        if (bots.Count == 0)
        {
            await ReplyAsync("当前没有连接的机器人。").ConfigureAwait(false);
            return;
        }
        ...
        await ReplyAsync($"屏幕已设置为 {(on ? "开" : "关")}，成功操作 {successCount}/{bots.Count} 台机器人。").ConfigureAwait(false);
    }
    ...
    public async Task SetStickAsync(SwitchStick s, short x, short y, ushort ms = 1_000)
    {
        ...
        if (bot == null)
        {
            await ReplyAsync($"没有可执行该指令的机器人：{s}").ConfigureAwait(false);
            return;
        }
    ...
    private async Task ClickAsyncImpl(SwitchButton button, BotSource<PokeBotState> bot)
    {
        ...
        {
            await ReplyAsync($"未知的按键值：{button}").ConfigureAwait(false);
            return;
        }
