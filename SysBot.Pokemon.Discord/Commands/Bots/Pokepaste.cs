...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        [Summary("从指定的 Pokepaste URL 生成队伍并通过私信发送文件。")]
        public async Task GenerateTeamFromUrlAsync(string pokePasteUrl)
        {
            var generatingMessage = await ReplyAsync("正在生成并发送您的 Pokepaste 队伍。请稍候...");
            try
            {
                await Task.Run(async () =>
                {
                    ...
                    {
                        await ReplyAndDeleteAsync($"在 Pokepaste URL 中未找到有效的 Showdown 配置: {pokePasteUrl}", 10, generatingMessage).ConfigureAwait(false);
                        return;
                    }
                        ...
                        foreach (var set in showdownSets)
                        {
                            try
                            {
                                ...
                                using (var cts = new CancellationTokenSource(timeout))
                                {
                                    ...
                                    {
                                        await ReplyAndDeleteAsync($"生成 {GameInfo.Strings.Species[template.Species]} 时超时。跳过...", 10, generatingMessage).ConfigureAwait(false);
                                        continue;
                                    }
                                }
                                ...
                                {
                                    await ReplyAndDeleteAsync($"创建 {GameInfo.Strings.Species[template.Species]} 失败。跳过...", 10, generatingMessage).ConfigureAwait(false);
                                    continue;
                                }
...
#pragma warning restore CA1416 // Validate platform compatibility
                    ...
                    await Context.User.SendFileAsync(memoryStream, $"{title}.zip", text: "这是您的队伍！").ConfigureAwait(false);
...
#pragma warning restore CA1416 // Validate platform compatibility
                        ...
                        var embedBuilder = new EmbedBuilder()
                            ...
                            .WithAuthor(
                                ...
                                {
                                    author
                                        .WithName($"{Context.User.Username} 生成的队伍")
            ...
            {
                await ReplyAndDeleteAsync($"生成 Pokepaste 队伍时出错: {ex.Message}", 10, generatingMessage).ConfigureAwait(false);
            }
        }
