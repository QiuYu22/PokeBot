...
namespace SysBot.Pokemon.Discord
{
    public class MysteryEggModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task TradeMysteryEggAsync()
        {
            ...
            {
                await ReplyAsync("由于游戏不支持培育，《Let's Go 皮卡丘／伊布》无法使用神秘蛋。").ConfigureAwait(false);
                return;
            }
        ...
        public async Task BatchMysteryEggAsync([Summary("蛋的数量（1-4）")] int count = 2)
        {
            // LGPE does not support eggs/breeding
            var context = GetContext();
            if (context == EntityContext.None || typeof(T).Name == "PB7")
            ...
            {
                await Helpers<T>.ReplyAndDeleteAsync(Context,
                    $"蛋的数量无效。请输入 1 到 {maxEggs} 之间的数值。", 5);
                return;
            }
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        await Context.Channel.SendMessageAsync($"{Context.User.Mention} 未能生成任何神秘蛋，请稍后再试。");
                        return;
                    }
                    ...
                    if (failedCount > 0)
                    {
                        await Context.Channel.SendMessageAsync($"{Context.User.Mention} 警告：有 {failedCount} 个蛋生成失败，将继续处理 {batchEggList.Count} 个蛋。");
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    await Context.Channel.SendMessageAsync($"{Context.User.Mention} 处理你的批量神秘蛋请求时发生错误，请稍后再试。");
        ...
        private static async Task ProcessBatchMysteryEggs(SocketCommandContext context, List<T> batchEggList, int batchTradeCode, int totalEggs)
        {
            ...
            {
                await context.Channel.SendMessageAsync("你已经在队列中！").ConfigureAwait(false);
                return;
            }
            ...
            await context.Channel.SendMessageAsync($"{context.User.Mention} - 已将 {batchEggList.Count} 个神秘蛋的批量请求加入队列！位置：{position.Position}。预估等待：{baseEta:F1} 分钟。").ConfigureAwait(false);
        ...
        private static Embed CreateMysteryEggEmbed(SocketCommandContext context, T pk, int eggNumber, int totalEggs, int queuePosition)
        {
            var embedBuilder = new EmbedBuilder()
                ...
                .WithDescription("一个装着随机宝可梦的神秘蛋！")
                .WithImageUrl("https://raw.githubusercontent.com/hexbyt3/sprites/main/mysteryegg3.png")
                .WithFooter($"批量交易 {eggNumber}/{totalEggs}" + (eggNumber == 1 ? $" | 队列位置：{queuePosition}" : ""))
                .WithAuthor(new EmbedAuthorBuilder()
                    .WithName($"为 {context.User.Username} 准备的神秘蛋")
        ...
        private static string? GetHiddenAbilityName(ushort species, EntityContext context)
        {
            ...
            {
                LogUtil.LogSafe(ex, $"无法获取物种 {species} 的隐藏特性");
            }
        ...
        private async Task ProcessMysteryEggTradeAsync(int code)
        {
            ...
            if (mysteryEgg == null)
            {
                await ReplyAsync("未能生成合法的神秘蛋，请稍后再试。").ConfigureAwait(false);
                return;
            }
