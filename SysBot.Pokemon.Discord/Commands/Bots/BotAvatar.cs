...
namespace SysBot.Pokemon.Discord
{
    public class BotAvatar : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task SetAvatarAsync()
        {
            ...
            {
                var reply = await ReplyAsync("请附上一张 GIF 图片作为头像。"); // standard (boring) images can be set via dashboard
            ...
            {
                var reply = await ReplyAsync("请提供一张 GIF 图片。");
                await Task.Delay(60000);
                await userMessage.DeleteAsync();
                await reply.DeleteAsync();
                return;
