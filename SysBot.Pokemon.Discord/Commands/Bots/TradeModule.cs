...
public class TradeModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task ShowMedalsCommand()
    {
        ...
        if (totalTrades == 0)
        {
            await ReplyAsync($"{Context.User.Username}，你还没有完成任何交易，开始交易以获得你的第一枚勋章吧！");
            return;
        }
    ...
    [Command("trade")]
    [Alias("t")]
    [Summary("让机器人根据提供的 Showdown 配置与你进行宝可梦交易。")]
    ...
    [Summary("让机器人根据提供的 Showdown 配置与你进行宝可梦交易。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public Task TradeAsync([Summary("交易代码")] int code, [Summary("Showdown 配置")][Remainder] string content)
        => ProcessTradeAsync(code, content);

    [Command("trade")]
    [Alias("t")]
    [Summary("让机器人使用你提供的宝可梦文件进行交易。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public Task TradeAsyncAttach([Summary("交易代码")] int code, [Summary("忽略自动 OT")] bool ignoreAutoOT = false)
    {
    ...
    [Command("trade")]
    [Alias("t")]
    [Summary("让机器人使用你附加的文件进行交易。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public async Task TradeAsyncAttach([Summary("忽略自动 OT")] bool ignoreAutoOT = false)
    {
        var userID = Context.User.Id;
        var code = Info.GetRandomTradeCode(userID);
        var sig = Context.User.GetFavor();
    ...
    [Summary("让机器人隐藏交易信息与你进行宝可梦交易。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public Task HideTradeAsync([Summary("交易代码")] int code, [Summary("Showdown 配置")][Remainder] string content)
        => ProcessTradeAsync(code, content, isHiddenTrade: true);

    [Command("hidetrade")]
    [Alias("ht")]
    [Summary("让机器人隐藏交易信息，使用你提供的文件进行交易。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public Task HideTradeAsyncAttach([Summary("交易代码")] int code, [Summary("忽略自动 OT")] bool ignoreAutoOT = false)
    {
        var sig = Context.User.GetFavor();
    ...
    [Command("hidetrade")]
    [Alias("ht")]
    [Summary("让机器人隐藏交易信息，使用你附加的文件进行交易。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public async Task HideTradeAsyncAttach([Summary("忽略自动 OT")] bool ignoreAutoOT = false)
    {
        var userID = Context.User.Id;
        var code = Info.GetRandomTradeCode(userID);
        var sig = Context.User.GetFavor();
    ...
    [Command("tradeUser")]
    [Alias("tu", "tradeOther")]
    [Summary("让机器人与被提及的用户进行附加文件交易。")]
    [RequireSudo]
    public async Task TradeAsyncAttachUser([Summary("交易代码")] int code, [Remainder] string _)
    {
        if (Context.Message.MentionedUsers.Count > 1)
        {
            await ReplyAsync("提及的用户过多，请一次仅排队一名用户。").ConfigureAwait(false);
            return;
        }

        if (Context.Message.MentionedUsers.Count == 0)
        {
            await ReplyAsync("必须提及一名用户才能执行此操作。").ConfigureAwait(false);
            return;
        }
    ...
    [Summary("让机器人根据提供的宝可梦名称生成并交易蛋。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public async Task TradeEggAsync([Summary("交易代码")] int code, [Summary("Showdown 配置")][Remainder] string content)
    {
        var userID = Context.User.Id;
        if (!await Helpers<T>.EnsureUserNotInQueueAsync(userID))
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context,
                "你在队列中已有无法清除的交易请求，请等待处理完成。", 2);
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                if (pkm is not T pk)
                {
                    await Helpers<T>.ReplyAndDeleteAsync(Context, "糟糕！我没能为该配置生成蛋。", 2);
                    return;
                }
    ...
    [Command("fixOT")]
    [Alias("fix", "f")]
    [Summary("在检测到广告时，修正链接交换中展示的宝可梦的训练家信息与昵称。")]
    ...
    [Summary("在检测到广告时，修正链接交换中展示的宝可梦的训练家信息与昵称。")]
    [RequireQueueRole(nameof(DiscordManager.RolesFixOT))]
    public async Task FixAdOT([Summary("交易代码")] int code)
    {
        var userID = Context.User.Id;
        if (!await Helpers<T>.EnsureUserNotInQueueAsync(userID))
        {
    ...
    [Command("dittoTrade")]
    [Alias("dt", "ditto")]
    [Summary("让机器人根据指定的能力分布与语言交易百变怪。")]
    public async Task DittoTrade([Summary("由 \"ATK/SPA/SPE\" 或 \"6IV\" 组合构成")] string keyword,
    ...
    [Alias("dt", "ditto")]
    [Summary("让机器人根据指定的能力分布与语言交易百变怪。")]
    public async Task DittoTrade([Summary("交易代码")] int code,
        [Summary("由 \"ATK/SPA/SPE\" 或 \"6IV\" 组合构成")] string keyword,
        [Summary("语言")] string language, [Summary("性格")] string nature)
    {
        var userID = Context.User.Id;
        if (!await Helpers<T>.EnsureUserNotInQueueAsync(userID))
    ...
    private async Task ProcessDittoTradeAsync(int code, string keyword, string language, string nature)
    {
        ...
        if (pkm == null)
        {
            await ReplyAsync("合法化耗时过长，请稍后再试。");
            return;
        }
        ...
        {
        var reason = result == "超时" ? "该配置生成耗时过长。" : "无法根据该配置生成合法宝可梦。";
            var imsg = $"糟糕！{reason} 这是我为该百变怪给出的最佳尝试！";
        ...
        if (Info.Hub.Config.Trade.TradeConfiguration.EnableSpamCheck)
        {
            ...
            {
                await Helpers<T>.ReplyAndDeleteAsync(Context, "检测到宝可梦名称或训练家名称包含广告，这是不允许的。", 5);
                return;
            }
        }
    ...
    [Alias("it", "item")]
    [Summary("让机器人交易携带指定道具的宝可梦。")]
    public async Task ItemTrade([Summary("交易代码")] int code, [Remainder] string item)
    {
        var userID = Context.User.Id;
        if (!await Helpers<T>.EnsureUserNotInQueueAsync(userID))
        {
    ...
    private async Task ProcessItemTradeAsync(int code, string item)
    {
        ...
        if (pkm.HeldItem == 0)
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context, $"{Context.User.Username}，未识别到你输入的道具。", 2);
            return;
        }
        ...
        {
        var reason = result == "超时" ? "该配置生成耗时过长。" : "无法根据该配置生成合法宝可梦。";
            var imsg = $"糟糕！{reason} 这是我为 {species} 给出的最佳尝试！";
            await Context.Channel.SendPKMAsync(pkm, imsg).ConfigureAwait(false);
            return;
        }
    ...
    public async Task GetTradeListAsync()
    {
        ...
        await ReplyAsync("以下为当前正在等待的用户：", embed: embed.Build()).ConfigureAwait(false);
    }

    [Command("fixOTList")]
    [Alias("fl", "fq")]
    [Summary("显示 FixOT 队列中的用户列表。")]
    [RequireSudo]
    ...
    [Command("listevents")]
    [Alias("le")]
    [Summary("列出可用的事件文件，可按字母或关键字筛选，并通过私信发送列表。")]
    ...
    [Alias("brl")]
    [Summary("列出可用的对战用宝可梦文件，可按字母或关键字筛选，并通过私信发送列表。")]
    ...
    [Alias("er")]
    [Summary("从指定事件文件夹下载附件，并将其加入交易队列。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    ...
    [Alias("brr", "br")]
    [Summary("从指定文件夹下载对战用附件，并将其加入交易队列。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    ...
    [Command("batchTrade")]
    [Alias("bt")]
    [Summary("让机器人根据提供列表一次交易多只宝可梦（最多 4 笔交易）。")]
    [RequireQueueRole(nameof(DiscordManager.RolesTrade))]
    public async Task BatchTradeAsync([Summary("使用 '---' 分隔的 Showdown 配置列表")][Remainder] string content)
    {
        ...
        {
            await Helpers<T>.ReplyAndDeleteAsync(Context,
                $"一次最多只能处理 {maxTradesAllowed} 笔交易，请减少批量交易中的条目数量。", 5);
            return;
        }

        var processingMessage = await Context.Channel.SendMessageAsync($"{Context.User.Mention} 正在处理你包含 {trades.Count} 只宝可梦的批量交易...");
        ...
        _ = Task.Run(async () =>
        {
            ...
            catch (Exception ex)
            {
                ...
                await Context.Channel.SendMessageAsync($"{Context.User.Mention} 处理你的批量交易时发生错误，请稍后重试。");
                Base.LogUtil.LogError($"批量交易处理出错：{ex.Message}", nameof(BatchTradeAsync));
            }
        });
    ...
    private async Task ProcessTradeAsync(int code, string content, bool isHiddenTrade = false)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                LogUtil.LogSafe(ex, nameof(TradeModule<T>));
                var msg = "糟糕！处理这个 Showdown 配置时发生了意外问题。";
