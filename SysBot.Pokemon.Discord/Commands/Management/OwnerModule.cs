...
public class OwnerModule<T> : SudoModule<T> where T : PKM, new()
{
    ...
    public async Task ListGuilds(int page = 1)
    {
        ...
        await ReplyAsync($"{Context.User.Mention}，我已通过私信向您发送了服务器列表（第 {page} 页）。");
    ...
    public async Task BlacklistServer(ulong serverId)
    {
        ...
        if (server == null)
        {
            await ReplyAsync("找不到具有提供 ID 的服务器。请确保机器人是您要加入黑名单的服务器的成员。");
            return;
        }

        var newServerAccess = new RemoteControlAccess { ID = serverId, Name = server.Name, Comment = "已列入黑名单的服务器" };

        settings.ServerBlacklist.AddIfNew([newServerAccess]);

        await server.LeaveAsync();
        await ReplyAsync($"已离开服务器 '{server.Name}' 并将其添加到黑名单。");
    }

    [Command("unblacklistserver")]
    [Alias("ubls")]
    [Summary("从机器人的服务器黑名单中移除服务器 ID。")]
    [RequireOwner]
    public async Task UnblacklistServer(ulong serverId)
    {
        ...
        if (wasRemoved)
        {
            await ReplyAsync($"ID 为 {serverId} 的服务器已从黑名单中移除。");
        }
        else
        {
            await ReplyAsync("尝试从黑名单中移除服务器时发生错误。请检查服务器 ID 后重试。");
        }
    }

    [Command("addSudo")]
    [Summary("将提及的用户添加到全局 sudo")]
    [RequireOwner]
    ...
    public async Task SyncChannels()
    {
        ...
        if (changesMade)
        {
            await ReplyAsync("频道白名单已成功与公告频道同步。").ConfigureAwait(false);
        }
        else
        {
            await ReplyAsync("白名单中的所有频道都已在公告频道中，无需更改。").ConfigureAwait(false);
        }
    }

    [Command("removeChannel")]
    [Summary("从接受命令的频道列表中移除频道。")]
    [RequireOwner]
    ...
    [Command("leaveguild")]
    [Alias("lg")]
    [Summary("根据提供的 ID 离开服务器。")]
    [RequireOwner]
    public async Task LeaveGuild(string userInput)
    {
        if (!ulong.TryParse(userInput, out ulong id))
        {
            await ReplyAsync("请提供有效的服务器 ID。").ConfigureAwait(false);
            return;
        }
        ...
        if (guild is null)
        {
            await ReplyAsync($"提供的输入 ({userInput}) 不是有效的服务器 ID，或者机器人不在指定的服务器中。").ConfigureAwait(false);
            return;
        }
    ...
    public async Task RePeek()
    {
        ...
        if (bot == null)
        {
            await ReplyAsync($"未找到具有指定 IP 地址 ({ip}) 的机器人。").ConfigureAwait(false);
            return;
        }
        ...
        {
            await ReplyAsync($"获取像素时出错: {ex.Message}");
            return;
        }
        ...
        var embed = new EmbedBuilder { ImageUrl = $"attachment://{img}", Color = (DiscordColor?)Color.Purple }
            .WithFooter(new EmbedFooterBuilder { Text = "这是您的截图。" });
    ...
    [Command("video")]
    [Alias("video")]
    [Summary("从当前配置的 Switch 获取并发送 GIF。")]
    [RequireSudo]
    public async Task RePeekGIF()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < screenshotCount; i++)
            {
                ...
                {
                    await ReplyAsync($"获取像素时出错: {ex.Message}").ConfigureAwait(false);
                    return;
                }
            ...
            await using (var ms = new MemoryStream())
            {
                ...
                var embed = new EmbedBuilder { ImageUrl = $"attachment://{gifFileName}", Color = (DiscordColor?)Color.Red }
                    .WithFooter(new EmbedFooterBuilder { Text = "这是您的 GIF。" });
...
#pragma warning restore CA1416 // Validate platform compatibility
    ...
    {
        await Context.Channel.EchoAndReply("正在关闭...再见！**机器人服务即将离线。**").ConfigureAwait(false);
        Environment.Exit(0);
    }
    ...
    public async Task DMUserAsync(SocketUser user, [Remainder] string message)
    {
        ...
        try
        {
            ...
            var confirmationMessage = await ReplyAsync($"消息已成功发送给 {user.Username}。");
        ...
        {
            await ReplyAsync($"发送消息给 {user.Username} 失败。错误: {ex.Message}");
        }
    }
    ...
    public async Task SayAsync([Remainder] string message)
    {
        ...
        {
            await ReplyAsync("请使用 #频道 正确提及频道。");
            return;
        }
        ...
        if (channel == null)
        {
            await ReplyAsync("未找到频道。");
            return;
        }

        if (channel is not IMessageChannel messageChannel)
        {
            await ReplyAsync("提及的频道不是文本频道。");
            return;
        }
        ...
        await ReplyAsync($"消息已成功发布在 {channelMention}。");
    }
